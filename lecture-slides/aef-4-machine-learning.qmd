...
&=\sigma_\varepsilon^2 E\left(D^{-1}\right)
\end{aligned}
$$
...
- Use the slider to explore in-sample vs. out-of-sample performance for different $P$
...
ui <- fluidPage(
  sidebarLayout(
    sidebarPanel(
      sliderInput("degree", "Polynomial degree (P):",
                  min = 1, max = in_sample_size - 1, value = 1, step = 1),
      helpText("Shaded area = out-of-sample period."),
      tableOutput("mse_table")
    ),
    mainPanel(plotOutput("poly_plot", height = "380px"))
  )
)
...
server <- function(input, output, session) {
  res <- reactive({
    ...
    list(
      pred_df    = tibble(date = data$date, fitted = c(preds_train, preds_test)),
  ...
  output$poly_plot <- renderPlot({
    ...
    ggplot(data, aes(x = date, y = returns)) +
      annotate("rect",
               ...
               ymin = -Inf, ymax = Inf, alpha = 0.25, fill = "grey70") +
      geom_point() +
      geom_line(data = r$pred_df, aes(x = date, y = fitted),
                color = "#2166ac", linewidth = 1) +
      scale_x_date(date_labels = "%Y-%m") +
      scale_y_continuous(labels = scales::percent) +
      labs(title    = paste0("P = ", input$degree),
           subtitle = sprintf("In-sample MSE: %.4f  |  OOS MSE: %.4f",
                              r$mse_train, r$mse_test),
           x = NULL, y = "Return") +
      theme_bw()
  })
  ...
  output$mse_table <- renderTable({
    r <- res()
    tibble(Period = c("In-sample", "Out-of-sample"),
           MSE    = c(r$mse_train, r$mse_test))
  }, digits = 4)
}
...
- Impose a penalty on the $L_2$ norm of the parameters $\hat\beta$ such that for $c\geq 0$ the estimator is 
...
$$\hat\beta^\text{ridge} = \left(D + \lambda I\right)^{-1}X'y$$
...
&= \left(I + \lambda D^{-1}\right)^{-1}D^{-1}D\hat\beta^\text{ols} = \left(I + \lambda D^{-1}\right)^{-1}\hat\beta^\text{ols}
\end{aligned}
$$
...
- For a thorough discussion of backpropagation, consider watching [this lecture by 3Blue1Brown](https://www.youtube.com/watch?v=Ilg3gGewQ5U)
